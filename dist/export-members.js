...
// Usage :
//   SUPABASE_URL=... SUPABASE_SERVICE_ROLE_KEY=... node export-members.js --table profiles
//   (ou via un .env : node -r dotenv/config export-members.js --table profiles)
// Options :
...
function toCsv(rows) {
    ...
    const lines = rows.map((r) => headers.map((h) => escapeCsvField(valueToString(r[h]))).join(';'));
...
(async () => {
    try {
        ...
        try {
            ...
            console.warn('⚠️  Échec avec colonnes prédéfinies, tentative avec SELECT * pour déduire la structure…');
